...
首先需要安装 **Economy 经济系统插件**（用于积分购买功能）：

📦 **Economy 插件下载**：https://github.com/SwiftlyS2-Plugins/Economy

按照 Economy 插件的说明完成安装和配置，特别是：

- 配置好 Economy 插件的数据库连接信息
- 确保 Economy 插件正常运行

### 2. 安装 PlayersModel 插件

1. **下载**最新版本的 PlayersModel 插件
2. **解压**到 SwiftlyS2 插件目录：
   ```
   csgo/addons/swiftly/plugins/PlayersModel/
   ```
3. **重启**服务器或使用 SwiftlyS2 命令加载插件

### 3. 数据库配置说明

> ⚠️ **重要提示**：PlayersModel 插件默认使用 Economy 插件的数据库连接键 `economyapi`。
>
> 只要你正确配置了 Economy 插件的数据库连接，PlayersModel 就会自动使用相同的数据库连接，无需额外配置。
